...
struct PID_Simple InitPIDController;
struct PID_Simple *PIDController = &InitPIDController;
int mode = 0;
void ITTimer_Config(){
...
void InitCtrl(struct MFA_Simple *ctrl){
	ctrl->N = NEURON_NUM; ctrl->eta = 0.2f; ctrl->Kc = 0.065f; ctrl->alpha = 0.02f;
...
void InitPIDCtrl(struct PID_Simple *ctrl){
	ctrl->Kp = 5.0f; ctrl->Ki = 30.0f; ctrl->Kd = 0.1f;
	ctrl->P = 0.0f;  ctrl->I = 0.0f;  ctrl->D = 0.0f;
	ctrl->T = Ts;
	ctrl->o[0] = 4200.0f; ctrl->o[1] = 4200.0f;
	ctrl->e[0] = 0.0f; ctrl->e[1] = 0.0f; ctrl->e[2] = 0.0f;
}
...
void PID_Control(struct PID_Simple *ctrl, float in_e){
	ctrl->P = ctrl->Kp*(ctrl->e[0] - ctrl->e[1]);
	ctrl->I = ctrl->Ki*ctrl->T/2*(ctrl->e[0] + ctrl->e[1]);
	ctrl->D = ctrl->Kd/ctrl->T*(ctrl->e[0] - 2*ctrl->e[1] + ctrl->e[2]);
	if (ctrl->e[0]<=1&&ctrl->e[0]>=-3)
		ctrl->o[0] = ctrl->o[0];
	else{
		ctrl->o[0] = ctrl->o[1] + ctrl->P + ctrl->I + ctrl->D;
	}
	TIM1->CCR1 = (int)ctrl->o[0];
}
