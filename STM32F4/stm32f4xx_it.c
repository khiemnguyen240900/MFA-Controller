...
//						GPIO_SetBits(GPIOD, GPIO_Pin_12);
								...
								if (m_data.myfloat <= 400) {
									Motor->yr_raw = m_data.myfloat;
									if (mode == 0){
										if (Motor->yr_raw < (Motor->y-50.0f)) mode = 1;
										else mode = 0;
									}
									else if (mode == 1){
										if (Motor->yr_raw > (Motor->y+50.0f)) mode = 1;
										else mode = 1;
									if (mode == 1) Motor->yr_raw -= 2; 
									};
								}
					}
			}
...
void TIM3_IRQHandler(void){
		...
		if (TIM_GetITStatus(TIM3, TIM_FLAG_Update) == SET){
			...
			PID_Update(PIDController, Motor->e);
			UpdateErrorCtrl(Controller, Motor->e);			
			if (mode == 0){				
				MFA_Algorithm(Controller, Motor->e, Motor->yr);
				PWM_Out(Controller->v);
			}
			else if (mode == 1){
				PID_Control(PIDController, Motor->e);
			}
			Update_Value(Motor);
